...
| ONTAP | ONTAP 9.1, 9.3, 9.5, 9.6, 9.7, 9.8P1 이상 


| VMware vSphere, vCenter Server, ESXi 호스트, SRM(Site Recovery Manager), 플러그인 애플리케이션 및 데이터베이스 열 1 | 를 참조하십시오 https://imt.netapp.com/matrix/imt.jsp?components=99343;&solution=1777&isHWU&src=IMT["상호 운용성 매트릭스 툴"^] 
|===
...
* VVOL 대시보드를 보려면 ONTAP 9.6 이하 버전용 OnCommand API 서비스를 다운로드하여 설치하십시오.
ONTAP 9.7 이상을 사용하는 경우 VASA Provider를 통해 OnCommand API 서비스를 등록할 필요가 없습니다.
 ...
 * 을(를) 다운로드했습니다 `.msi` SRM(Site Recovery Manager) 재해 복구 솔루션을 구성하려는 경우에만 SRA 플러그인에 대한 파일입니다. 열 1
...
.단계
. 를 다운로드합니다 `.ova` 에서 파일 https://mysupport.netapp.com/site/products/all/details/otv/downloads-tab["NetApp Support 사이트"^] ONTAP 툴을 구축하는 vSphere 클라이언트 시스템으로
+
...
.단계
. 를 다운로드합니다 `.msi` NetApp Support 사이트에서 SRA 플러그인용 설치 프로그램
. 다운로드한 을 두 번 클릭합니다 `.msi` SRA 플러그인용 설치 프로그램 및 화면의 지침을 따릅니다.
...
. ONTAP 도구 IP 주소 및 암호를 사용하여 SRM 구성: `perl command.pl -I <va-IP> administrator <va-password>`
